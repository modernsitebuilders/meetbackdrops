...
// /collections/{slug} canonical. These are NEW indexable URLs that aggregate
// cross-category curated sets with unique copy — not duplicates of category
// pages (which own the individual images).
function collectionSeo(def, items) {
  ...
  return {
    ...
    description: def.description,
    canonical,
    keywords: `${def.persona.toLowerCase()} virtual backgrounds, zoom backgrounds, teams backgrounds`,
    ogImage,
    schema,
  };
}
