...
/**
 * The descriptive head of a stored manifest title, with the brand suffix and
 * the templated marketing tail removed and NO length trim.
 *
 * This is the on-page display name — the <h1>, the breadcrumb leaf, schema
 * `name` fields. It is deliberately separate from buildImagePageTitle(): that
 * one serves the <title> tag and must fit a 65-char SERP budget, which is not
 * a constraint on a heading. Rendering `image.title` raw instead puts a
 * literal " | MeetBackdrops" in the middle of the page.
 */
function buildImageDisplayTitle(rawTitle) {
  ...
  return head || 'Virtual Background';
}
...
function buildImagePageTitle(rawTitle) {
  const head = buildImageDisplayTitle(rawTitle);
  if (head === 'Virtual Background') return `Virtual Background${BRAND_SUFFIX}`;
