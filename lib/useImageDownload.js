...
export function useImageDownload(cloudinaryUrls) {
  ...
  const handleDownload = async (image, category) => {
    ...
    try {
     ...
     await trackDownloadToSheets(image, category, session, isAdmin);
  ...
  const triggerDownloadWithRateLimitHandling = async (image, category, session, skipLimits) => {
    ...
    console.log('🔍 Looking up:', baseFilename, '| Found:', !!cloudinaryUrls?.[baseFilename], '| Total keys:', Object.keys(cloudinaryUrls || {}).length);
