...
export function useImageDownload(cloudinaryUrls) {
  ...
  const handleEmailBonus = async (email) => {
    ...
    const pngFilename = `MeetBackdrops-${baseFilename}.png`;
    const cloudinaryUrl = cloudinaryUrls?.[baseFilename]
      ? cloudinaryUrls[baseFilename]
      : `https://assets.streambackdrops.com/${baseFilename}.png`;

    fetch('/api/analytics', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ eventType: 'email_bonus_download', email, category, filename: pngFilename }),
    }).catch(() => {});

    fetch('/api/save-email', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, source: 'bonus_download' }),
    }).catch(() => {});
    ...
    try {
      const folder = image.folder || category;
      const webpResponse = await fetch(`https://assets.streambackdrops.com/webp/${folder}/${image.filename}`);
      const blob = await webpResponse.blob();
      const blobUrl = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = blobUrl;
      link.download = `MeetBackdrops-${baseFilename}.webp`;
      link.style.display = 'none';
      document.body.appendChild(link);
      link.click();
      setTimeout(() => { document.body.removeChild(link); URL.revokeObjectURL(blobUrl); }, 1000);
  ...
  const triggerDownloadWithRateLimitHandling = async (image, category, session, skipLimits) => {
    ...
    if (cloudinaryUrls && cloudinaryUrls[baseFilename]) {
      cloudinaryPngUrl = cloudinaryUrls[baseFilename];
    }

    if (!cloudinaryPngUrl) {
      cloudinaryPngUrl = `https://assets.streambackdrops.com/${baseFilename}.png`;
    }
    ...
    if (!response.ok) {
      ...
      const webpFilename = `MeetBackdrops-${baseFilename}.webp`;
      const folder = image.folder || category;
      const webpResponse = await fetch(`https://assets.streambackdrops.com/webp/${folder}/${image.filename}`);
