...
const performSearch = (keywords) => {
    ...
    console.log('🔤 Search terms (lowercase):', searchLower);
    const matchedImages = [];
    const seenIds = new Set(); // Track seen image IDs to prevent duplicates

    console.log('📦 Total images in metadata:', Object.keys(imageMetadata).length);
    ...
    Object.keys(imageMetadata).forEach((key) => {
      ...
      // Only include images that match ALL selected keywords (AND logic)
    ...
    console.log('✅ Found matches:', matchedImages.length);
    console.log('📸 First 3 results:', matchedImages.slice(0, 3));
