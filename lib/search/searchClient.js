...
// Category-level aliases: search words that should surface an ENTIRE category
// even when the word appears in no image's title/tags. Injected into the
// category haystack (see prepareIndex) so a single-token query like "cafe"
// matches every image in the category — NOT routed through SYNONYMS, because
// the AND-match logic in search() would let the few images that literally say
// "cafe" suppress the rest of the category.
const CATEGORY_ALIASES = {
  'coffee-shops': 'cafe cafes coffeehouse',
};
...
export function prepareIndex(raw) {
  ...
  const images = raw.images.map((r) => {
    ...
    const catText = `${r.c} ${categoryLabel.get(r.c) || ''} ${CATEGORY_ALIASES[r.c] || ''}`;
