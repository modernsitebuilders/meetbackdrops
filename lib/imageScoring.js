...
// NEW: Match filenames accounting for MeetBackdrops- prefix and different extensions
export function matchFilenames(filename1, filename2) {
  if (!filename1 || !filename2) return false;
  
  // Remove MeetBackdrops- or legacy StreamBackdrops- prefix
  const clean1 = filename1.replace('MeetBackdrops-', '').replace('StreamBackdrops-', '');
  const clean2 = filename2.replace('MeetBackdrops-', '').replace('StreamBackdrops-', '');
...
export function findMatchingAnalytics(filename, analyticsData) {
  ...
  const variants = [
    ...
    `${baseName}.jpg`,
    `${baseName}.jpeg`,
    `MeetBackdrops-${baseName}.webp`,
    `MeetBackdrops-${baseName}.png`,
    `MeetBackdrops-${baseName}.jpg`,
    `MeetBackdrops-${baseName}.jpeg`,
...
export function calculateAllScores(images, metadataMap = {}, analyticsData = {}) {
  ...
  images.forEach(image => {
    ...
    // Find matching analytics (handles MeetBackdrops- prefix and .png extensions)
