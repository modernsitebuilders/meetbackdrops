...
export function getVisitorType() {
  ...
  try {
    ...
    const hoursSinceFirst = (Date.now() - parseInt(firstVisit)) / (60 * 60 * 1000);
    
    if (hoursSinceFirst < 6) return 'new';
    if (hoursSinceFirst < 23) return 'returning';
    if (hoursSinceFirst < 144) return 'daily'; // 6 days
    if (hoursSinceFirst < 720) return 'weekly'; // 30 days
    return 'monthly';
  } catch (e) {
    return 'new';
  }
