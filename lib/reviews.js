...
export async function getReviewsData() {
  try {
    ...
    // Parse reviews - ONLY INCLUDE APPROVED REVIEWS
    const reviews = rows.map(row => ({
      ...
      review.rating && // Has a rating
      review.status && // Has a status
      review.status.toLowerCase() === 'approved' // Only approved reviews
    );

    // Calculate aggregate rating (only from approved reviews)
