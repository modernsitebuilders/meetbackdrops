...
export default function CategoryPage({ slug, scores, metadata = {} }) {
  ...
  : String(category.seoDescription || `Download free ${categoryName.toLowerCase()} backgrounds. Perfect for Zoom, Teams & Google Meet.`);
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    // ── Primary: read from pre-generated static scores file (Jan 25+ data only) ──
    // This file is generated by scripts/generate-scores.js and committed to the repo.
    // It gives immediate correct ordering on every deploy without waiting for ISR.
    ...
    if (fs.existsSync(staticScoresPath)) {
      try {
        ...
        staticLoaded = Object.keys(scores).length > 0;
      } catch (e) {
        console.error('Failed to read static scores file:', e.message);
      }
    }

    // ── Fallback: fetch live scores from Google Sheets API ──
    // Used when static file is missing or empty (e.g. fresh clone without the file).
    if (!staticLoaded) {
