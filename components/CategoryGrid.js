...
export default function CategoryGrid({ navigate }) {
  return (
    <div id="categories" className={`${styles.categoryGrid} category-grid`}>
      ...
      <Card href="/category/nature-landscapes" title="Nature & Landscapes" description="Stunning natural landscapes and scenic outdoor views for nature-inspired calls" imageSrc={`${CDN}/nature-landscapes/nature-landscape-01.webp`} imageAlt="Nature landscape virtual background" navigate={navigate} count={CATEGORIES['nature-landscapes'].count} />
      <Card href="/category/libraries" title="Libraries" description="Classic library rooms with floor-to-ceiling books" imageSrc={`${CDN}/libraries/library-01.webp`} imageAlt="Library virtual background" navigate={navigate} count={CATEGORIES['libraries'].count} />
