...
export default function CategoryGrid({ navigate }) {
  return (
    <div id="categories" className={`${styles.categoryGrid} category-grid`}>
      ...
      <Card href="/category/neutral-backgrounds" title="Neutral & Plain Walls" description="Minimalist neutral and plain wall backgrounds — off-white, greige & soft gray — for clean, distraction-free calls" imageSrc={`${CDN}/neutral-backgrounds/seamless-video-call-background-d4a783bf.webp`} imageAlt="Neutral plain wall virtual background for video calls" navigate={navigate} count={CATEGORIES['neutral-backgrounds'].count} />
